...
class RlWalkingCustomExampleParam : public BasicParam {
 public:
  RlWalkingCustomExampleParam(std::string_view tag = "rl_walking_custom_example") : BasicParam(tag) {
  ...
  std::optional<bool> LOAD_PARAM(upper_body_lock_enabled);
  std::optional<Eigen::VectorXd> LOAD_PARAM(upper_body_lock_joint_q);
  std::optional<double> LOAD_PARAM(upper_body_lock_interpolation_duration);
