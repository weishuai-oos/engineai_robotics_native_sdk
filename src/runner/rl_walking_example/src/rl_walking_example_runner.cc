 ...
 * @brief Implementation of the RL (Reinforcement Learning) walking policy
 * Runner.
 *
 * This file implements the deployment of an RL-trained walking policy on a real
 * humanoid robot. The core control loop follows a standard
 * perception-decision-action pipeline each cycle:
 *   1. Read sensor data (IMU, joint encoders) to capture the robot's current
 * state
 *   2. Assemble the state into an observation vector matching the RL training
 * environment
 *   3. Feed the observation into an MLP policy network for forward inference to
 * get an action
 *   4. Convert the action into target joint positions and send them to the
 * motors via PD control
 *
 * Notes for secondary developers:
 *   - If you need to modify the observation composition (e.g., adding new
 * sensor inputs), you MUST update CalculateObservation() AND the training-side
 * observation definition simultaneously, otherwise an input dimension mismatch
 * will occur.
 *   - The policy model file (.mnn format) path is configured via the
 * `policy_file` parameter and must match the model exported from the training
 * side.
 *   - Joint mapping is determined by the `active_joint_names` parameter, which
 * defines which subset of joints the RL policy controls and their ordering.
 * This MUST be identical to the training-side configuration.
 */
 ...
 * @param name       Runner identifier, passed in by the runner registration
 * macro (e.g., "rl_walking_example_runner").
 * @param data_store Shared data bus for exchanging sensor data, control
 * commands, and other information between this Runner and other system modules.
 *
 * @note `param_` is created via ParamManager and automatically loads parameters
 *       from the corresponding YAML configuration file. To use different
 * parameter sets for different robot models or scenarios, switch via param_tag_
 * (see Enter()).
 */
RlWalkingExamplePm01Runner::RlWalkingExamplePm01Runner(
    std::string_view name, const std::shared_ptr<data::DataStore> &data_store)
    : MotionRunner(name, data_store) {
  // Create the RL walking parameter instance using the default tag (loaded from
  // config)
...
/**
 * @brief Sets up the runtime context, called before the Runner is scheduled to
 * run.
 *
 * Sets `parallel_by_classic_parser` to false, indicating that this Runner does
 * NOT use the classic parser's parallel motion control mode — the RL policy has
 * exclusive control over all active joints.
 */
void RlWalkingExamplePm01Runner::SetupContext() {
  data_store_->parallel_by_classic_parser.store(false);
}

/**
 * @brief Tears down the runtime context, called after the Runner is
 * unscheduled. Currently no additional cleanup is required.
 */
void RlWalkingExamplePm01Runner::TeardownContext() {}

/**
 * @brief Initialization logic executed upon entering this Runner. Allocates
 * resources and resets all internal state.
 *
 * This method performs the following steps:
 *   1. Hot-switch the parameter set if param_tag_ has changed (supports runtime
 * config switching)
 *   2. Load the MLP policy network model (.mnn file)
 *   3. Initialize the observation buffer and action buffer
 *   4. Initialize the low-pass filter for remote control commands (smooths
 * gamepad input)
 *   5. Build the "policy joint index → full-body joint index" mapping
 *   6. Load default joint positions, PD gains (Kp/Kd), and action scale
 * coefficients
 *   7. Record the current joint positions as the starting point for smooth
 * interpolation
 *
 * @return true if initialization succeeds and the Runner is ready to run.
 */
bool RlWalkingExamplePm01Runner::Enter() {
  // --- Step 1: Hot-switch parameters ---
  // If an external module has set a new param_tag_ (e.g., via the task system),
  // reload parameters
  if (!param_tag_.empty() && param_tag_ != last_param_tag_) {
    param_ =
        data::ParamManager::create<data::RlWalkingExampleParam>(param_tag_);
    last_param_tag_ = param_tag_;
  }

  // --- Step 2: Load the MLP policy network ---
  // policy_file is the model file path relative to the config directory (.mnn
  // format). MNNModel wraps the MNN inference framework for forward inference.
  ...
  // Columns represent time steps, supporting multi-step historical observation
  // stacking (used for temporal features in the policy network).
  mlp_net_observation_.setZero(param_->num_observations,
                               param_->num_include_obs_steps);
  // Action vector dimension: [num_actions], i.e., the number of joints
  ...
  lpf_command_ =
      std::make_unique<math::FirstOrderLowPassFilter<Eigen::Vector3d>>(
          param_->remote_command_sampling_frequency,
          param_->remote_command_cut_off_frequency);

  // --- Step 5: Build joint index mapping ---
  // active_joint_idx_ maps policy action indices to actual indices in the
  // full-body joint array. For example, the policy may only control 12 joints
  // out of 20+ total joints; this mapping tells us which full-body joint
  ...
  // Joints listed in disable_transition_joints skip startup position/PD
  // blending.
  disable_transition_joint_idx_.resize(0);
  if (param_->disable_transition_joints.has_value()) {
    const auto &names = param_->disable_transition_joints.value();
    disable_transition_joint_idx_.resize(static_cast<int>(names.size()));
    int j = 0;
    for (const std::string &name : names) {
      disable_transition_joint_idx_(j++) =
          model_param_->joint_id_in_total_limb.at(name);
    }
  }
  ...
  default_joint_q_ = common::ConcatenateVectors(
      param_->default_joint_q); // Default standing pose [rad]
  joint_kp_ = common::ConcatenateVectors(
  ...
  imu_install_bias_ = param_->imu_install_bias.value_or(
      Eigen::Vector3d::Zero()); // IMU mounting bias compensation
 ...
 *
 * The call frequency is determined by the system scheduler and typically
 * corresponds to param_->control_dt (e.g., 50Hz or 100Hz).
 *
 ...
 */
void RlWalkingExamplePm01Runner::Run() {
 ...
 *
 * When the system requests this Runner to exit, this method is called
 * repeatedly:
 ...
 *
 * This design ensures the robot does not abruptly lose control during the exit
 * process.
 */
TransitionState RlWalkingExamplePm01Runner::TryExit() {
...
/**
 * @brief Cleanup logic after exiting the Runner. Currently no additional
 * actions needed.
 */
bool RlWalkingExamplePm01Runner::Exit() { return true; }

/**
 * @brief Called when the Runner terminates. Currently no additional actions
 * needed.
 */
void RlWalkingExamplePm01Runner::End() {}
 ...
 * (common requirement since forward walking is typically more stable than
 * backward).
 *
 * An optional low-pass filter can be enabled to smooth sudden input changes
 * from the gamepad, preventing velocity command spikes that could destabilize
 * the robot.
 */
void RlWalkingExamplePm01Runner::UpdateRemoteCommand() {
  const auto &gamepad = data_store_->gamepad_info.Get();

  // Select positive/negative scaling factor based on stick direction
  command_.x() = (gamepad->LeftStick_X >= 0)
                     ? gamepad->LeftStick_X * param_->command_scale_pos.x()
                     : gamepad->LeftStick_X * param_->command_scale_neg.x();
  command_.y() = (gamepad->LeftStick_Y >= 0)
                     ? gamepad->LeftStick_Y * param_->command_scale_pos.y()
                     : gamepad->LeftStick_Y * param_->command_scale_neg.y();
  command_.z() = (gamepad->RightStick_Y >= 0)
 ...
 *   1. Compute the robot body orientation from IMU data (with mounting bias
 * compensation)
 ...
 * The ordering of components MUST match the training environment definition
 * exactly:
 *
 *   | Component                     | Dimension        | Description |
 *   |-------------------------------|------------------|-------------------------------------------|
 *   | q_real - default_joint_q      | num_actions      | Joint position offset
 * from default [rad]  | | qd_real                       | num_actions      |
 * Joint angular velocity [rad/s]            | | mlp_net_action (previous)     |
 * num_actions      | Previous step's policy action output      | |
 * angular_velocity (body frame) | 3                | IMU angular velocity in
 * body frame [rad/s]| | projected_gravity             | 3                |
 * Gravity projection in body frame          |
 *
 * @warning Modifying the observation composition or ordering requires
 * corresponding changes on the training side and model retraining.
 */
void RlWalkingExamplePm01Runner::CalculateObservation() {
  // --- 1. IMU orientation computation (with mounting bias compensation) ---
  // R_install: Rotation matrix for IMU mounting bias (compensates for the
  // physical
  //            mounting angle offset between the IMU and its theoretical
  //            position)
  Eigen::Matrix3d R_install =
      math::RotationMatrixd(math::RollPitchYawd(imu_install_bias_)).matrix();
  // R_local: Raw orientation reported by the IMU (world frame → IMU frame)
  Eigen::Matrix3d R_local =
      math::RotationMatrixd(data_store_->imu_info.Get()->quaternion).matrix();
  ...
  Eigen::Vector3d w_real = R_real.transpose() * R_local *
                           data_store_->imu_info.Get()->angular_velocity;
  Eigen::Vector3d euler_xyz =
      math::RollPitchYawd(math::RotationMatrixd(R_real)).vector();
  ...
  Eigen::Vector3d projected_gravity_real =
      -R_real.transpose() * Eigen::Vector3d::UnitZ();

  // --- 2. Concatenate the single-step observation vector ---
  // Note: The velocity command (command_) is NOT included here; it is
  // concatenated separately in CalculateMotorCommand() as the policy network's
  // command input.
  Eigen::VectorXd mlp_net_observation_single =
      Eigen::VectorXd::Zero(param_->num_observations);
  mlp_net_observation_single << //
      (q_real_ - default_joint_q_)(
          active_joint_idx_),      // Joint position offset [num_actions]
  ...
  // Scale by configured factors (matches the normalization used during
  // training)
  mlp_net_observation_single.array() *= param_->observation_scale.array();
  // Clip observations to prevent abnormal sensor values from destabilizing the
  // policy
  mlp_net_observation_single =
      mlp_net_observation_single.cwiseMax(-param_->observation_clip)
          .cwiseMin(param_->observation_clip);
 ...
 *   2. Concatenate multi-step historical observations (column-flattened) and
 * the velocity command into the final input vector
 *   3. Run MNN forward inference to obtain the action vector
 *   4. Clip the action (prevents unsafe outputs)
 *   5. Map action to target joint positions: q_des = default_q + action *
 * action_scale
 *
 * @note Policy network input vector layout:
 *       [obs_step_0, obs_step_1, ..., obs_step_N, command(3)]
 *       where each obs_step has dimension num_observations, giving a total
 * input dimension of: num_observations * num_include_obs_steps + 3.
 */
void RlWalkingExamplePm01Runner::CalculateMotorCommand() {
  ...
  Eigen::VectorXd obs;
  obs = Eigen::VectorXd::Zero(
      param_->num_observations * param_->num_include_obs_steps + 3);
  // Flatten the observation matrix into a 1D vector (transpose first because
  // Eigen uses column-major storage, but the network expects data ordered by
  // time step)
  obs.head(param_->num_observations * param_->num_include_obs_steps) =
      Eigen::Map<Eigen::VectorXd>(mlp_net_observation_.transpose().data(),
                                  mlp_net_observation_.size());
  // Append the velocity command at the tail of the observation vector
  obs.tail(3) = command_obs;

  // Run MLP forward inference (float precision for inference, cast back to
  // double)
  ...
  mlp_net_action_ = mlp_net_action_.cwiseMax(-param_->action_clip)
                        .cwiseMin(param_->action_clip);

  // Map action to target joint positions:
  //   q_des[active_joints] = default_q[active_joints] + action * action_scale
  // Non-active joints retain their default positions (not controlled by the
  // policy)
...
bool RlWalkingExamplePm01Runner::InitializeEntryTransition() {
  ...
  config.enabled = param_->entry_transition_enabled.value_or(legacy_enabled) && !direct_takeover;
  if (direct_takeover) {
    LOG(INFO) << "[RlWalkingExamplePm01Runner] Direct gait takeover from previous motion ["
              << previous_motion << "]";
  }
 ...
 *   - qd_des_:     Target joint velocities [rad/s] (set to 0, pure position
 * control)
 ...
 *       secondary developers can modify tau_ff_des_ before calling
 * SetCommand().
 */
